      ...
      <div class="helperContainer">
        <div class="line">
          ...
          >
            ...
            <span v-if="showYearMonth">{{ globalModel[idSelectedTab].name }}</span>
          </button>
          <button
            v-if="historyButton && previousSelected.length"
            @click="historyBack"
            class="historyButton"
          >
            {{ historyButtonText }}
        ...
        <div v-if="search" class="line" style="position: relative;">
          <input
            ...
            class="inputFilter"
          />
          <button type="button" class="clearButton" @click="clearSearch()">×</button>
        </div>
      </div>
